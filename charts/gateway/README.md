...
| apisix.image.tag | string | `"3.10.6"` | API7 Gateway image tag Overrides the image tag whose default is the chart appVersion. |
...
| apisix.streamPlugins | list | `[]` | Stream (L4) plugin list, rendered as `stream_plugins` in `config.yaml`. Leave it empty to keep the gateway image's own default, which does not enable `prometheus`. Set it to enable the L4 Prometheus metrics: the `apisix_stream_active_connections` and `apisix_stream_bandwidth` metrics need the shared memory zone that `apisix init` only renders when `prometheus` is in this list. |
